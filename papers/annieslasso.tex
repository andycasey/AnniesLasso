...
For the purposes of post-validation of our constructed model, we have assigned
each star in the labelled set a uniformly-distributed integer $q$ between 0
...
experiments, we have ensured that the same $q$ integer is assigned to each
...
\begin{equation}
y_j = \sum_{k=0}^{K} A_{2k}\sin{\left(\frac{2k\lambda_{j}\pi}{L}\right)} + A_{(2k+1)}\cos{\left(\frac{2k\lambda_{j}\pi}{L}\right)}
\end{equation}

We chose the length scale $L = 1400$, using $K = 3$, and solved for the
...
More importantly, the use of sine and cosine functions implies that edges of the
continuum function are going to be bounded (more bounded than a polynomial
function), and are therefore less susceptible to issues with overfitting 
where the edge of the continuum function will be unbounded.
