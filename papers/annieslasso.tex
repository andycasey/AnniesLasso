...
For the purposes of post-validation of our constructed model, we have randomly
...
for each observation. The error arrays themselves do not encapsulate all
knowledge about technical, observational, or reduction issues.  Instead, every
pixel from a single observation contains a bitmask flag that documents any of 
these issues.  Although flagged, these issues are not reflected in the error
arrays (i.e., by having larger error values).  For this reason we chose to 
increase the error arrays for pixels that are flagged.  Specifically for
flagged pixels we adjusted the inverse variance such that:

\begin{equation}
adjusted_{ivar} = \frac{ivar}{1 + \Delta^2 * ivar}
\end{equation}
...
pixels.  However we chose to ignore bitmask values associated with persistence 
...
We produced a distilled list of continuum pixels from the spectra (following
the initial identification in \citet{tc}), and for each observation we fit
the fluxes in those pixels (within a region) as a sum of sine and cosines:
...
We chose the length scale $L = 1400$, using $W = 3$, and solved for the
amplitudes $A$ in each region separately. 
...
Firstly, it is a linear operation on the spectrum fluxes and therefore cheap.
...
In the simplest case we have two hyper-parameters that still need to be 
...
coefficients (unless it is well-fit by the data) and thus enforce sparsity.  
High scale factor acts to penalize cross-terms moreso than the linear labels. 
...
$f$ from 0.5 to 50.  For each combination of $(\Lambda, f)$ we trained the full
17-label model and estimated labels for all (individual and stacked) spectra in
the validation set.


We must chose on a heuristic to select the optimal regularization $\Lambda$ and
...
label precision at low S/N, or opting to maximize the model sparsity (the
fraction of zero-value model coefficients).  In practice these metrics must be
balanced if we want a sparse (interpretable) model that yields precise labels
even at low S/N values.


We define the model sparsity as the percent of zero-value model coefficients.
...
not change (regardless of $f$) until $\Lambda \gtrsim 10^3$.  In this regime
the linear and second-order coefficient sparsity metrics exhibit very
different responses.  The cross-term sparsity changes far faster than the
...
sparsest model for the given regularization factor.


While sparser models are preferred, our ultimate goal is to have an
interpretable (sparse) model that predicts spectral fluxes and returns precise
...
% ARC The chi^2 value figure.


Our grid search has revealed that a regularization factor of $\Lambda = X$ and
a scale factor $f = Y$ yields: a relatively sparse (interpretable) model that
predicts spectral fluxes well (better than the unregularized case), and provides
precise stellar labels at low S/N.  We adopt these values for the remainder of
this work.
...
a disjoint \emph{validation set}.  After setting the hyper-parameters $\Lambda$
and $f$ and training the model, here we seek to understand the model performance
as a function of S/N ratio.  The stars in the validation set include stacked
spectra, and the individual normalized observations.  We apply our model to
all stacked and individual spectra and recover labels for every spectrum. 
...
metric for method performance.  Analysis pipelines that have larger uncertainties
