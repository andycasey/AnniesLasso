...
abundance labels.  The worst performance is found for [V/H], by a factor of
two: the second-worst label at $S/N \geq 50$ is [Na/H] with 0.08~dex. 
The [V/H] abundance labels are also problematic in the \aspcap\ pipeline 
...
for this is not completely understood (by us), because both approaches 
(optimizing a data-driven or physics-driven model) rely on least-squares 
fitting, which will typically follow the $1/\sqrt{S/N}$ behaviour we see in Figure 
...
in our open-source code for any polynomial vectorizer.
...
% 	 	Maybe we could just make the rogue stuff available at a separate DOI
%		and encourage others to look at the data because we can't be fucked?
...
Although our membership criteria may suffer from mild contamination by
...
in the same direction, but a smaller distribution spread is seen in our
labels.  The most striking difference is seen in M~15 (Figure 
\ref{fig:m15-comparison}).
