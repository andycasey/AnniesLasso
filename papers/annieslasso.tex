...
There are a number of important implications that arise from our 
globular cluster abundances.  First, it is critical to
note that the training set was \emph{not} constructed to purposefully include
stars from \emph{any} globular clusters.  We selected stars that met
strict quality criteria, and stars with questionable label fidelity 
were removed.  This is significant because \emph{the anti-correlations
seen in globular clusters here and elsewhere are not dominant in our
training set}.  For this reason the regularized model is measuring sets
of labels (e.g., a chemical fingerprint) that are \emph{very different}
to what it was actually trained on.  And the labels have good astrophysical
interpretations: although [C/Fe], [N/Fe] and [O/Fe] vary across the giant
branch due to CNO cycling, the sum of [(C,N,O)/3Fe] should remain roughly
constant for each globular cluster.  This is not the case in the \aspcap\
labels, where a huge spread in total CNO abundances is seen.  In contrast,
\TheCannon\ labels remain roughly constant for each globular cluster.  This
is a simple example where \TheCannon\ allows for an increased understanding
of astrophysics, directly through improved precision in abundance labels.
...
in a companion paper (Ness et al., 2016, in preparation).  %This implies that
%additional information (e.g., label masks) could improve the \emph{reliability}
%of labels, strictly by limiting the number of pixels providing covariant
%information.
