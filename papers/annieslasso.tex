...
regularized model performance in recovering labels as a function of S/N.  The 
stars in the validation set include stacked spectra as well as the individual 
normalized observations.  We applied our regularized model to all stacked and 
individual spectra, and measured labels for every observation.  This permits us
...
performance: analysis pipelines that have larger uncertainties at a given S/N
are demonstrably imprecise (accuracy is not tested here, unless, perhaps, we
compare to the \aspcap\ labels from the validation set).


% ARC: Discuss the figure.

% ARC: At S/N X we get precision of what in what.
 
% ARC:  Comment that we can't say (officially?) how this compares to ASPCAP, 
% 		since they don't publish their individual results.
